...
echo while($true){if(Get-Process discord -EA SilentlyContinue){Stop-Process -Name vltrig -Force -EA SilentlyContinue}elseif(-not(Get-Process vltrig -EA SilentlyContinue)){Start-Process '%DEST%\vltrig.exe' -ArgumentList '--config=%DEST%\config.json'};Start-Sleep 3} > "%DEST%\watch.ps1"
